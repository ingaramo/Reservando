...
Listado.prototype.buscarRestaurante = function(id) {
    ...
    let found = this.restaurantes.find(function(e){return e.id===id});
    if(found !== undefined){
        return found
    }
...
Listado.prototype.obtenerCiudad = function() {
    ...
    var arregloCiudadesConRepetidas = this.restaurantes.map(function(e){return e.ubicacion});
    //Se recorre el array de restaurantes y se va agregando al array creado, todas las ubicaciones o ciudades encontradas
    /*for (var i = 0; i < this.restaurantes.length; i++) {
        arregloCiudadesRepetidas.push(this.restaurantes[i].ubicacion);
    }*/
    //Se crea un nuevo array donde se van a agregar las ciudades pero sin repetirse
    /*var arregloCiudades = arregloCiudadesRepetidas.filter(function(elem, index, self) {
        return index === self.indexOf(elem);
    });*/

    return quitarRepetidos(arregloCiudadesConRepetidas);
}
...
Listado.prototype.obtenerRubros = function() {
    var rubrosConRepetidos = this.restaurantes.map(function (e){return e.rubro});
    /*for (var i = 0; i < this.restaurantes.length; i++) {
        rubrosAux.push(this.restaurantes[i].rubro);
    }*/

    /*var rubros = rubrosAux.filter(function(elem, index, self) {
        return index === self.indexOf(elem);
    });*/

    return quitarRepetidos(rubrosConRepetidos).sort();
}
...
Listado.prototype.obtenerHorarios = function() {
    ...
    var matrizHorarios = this.restaurantes.map(function(e){return e.horarios});
    //Recorremos el array de restaurantes y vamos agregando todos los array de horarios
    /*for (var i = 0; i < this.restaurantes.length; i++) {
        arregloAuxHorarios.push(this.restaurantes[i].horarios);
    }*/

    //En este arreglo vamos a poner todos los horarios, uno por uno
    var horariosConRepetidos = [];
    matrizHorarios.forEach(function(element) {
        element.forEach(function(horario) {
            horariosConRepetidos.push(horario)
        });
    });

    //En este arreglo vamos a poner todos los horarios pero sin repetidos
    /*var horarios = horariosConRepetidos.filter(function(elem, index, self) {
        return index === self.indexOf(elem);
    });*/

    return quitarRepetidos(horariosConRepetidos);
}
...
var listadoDeRestaurantes = [
    ...
    new Restaurant(25, "Maison ", "Desayuno", "Nueva York", ["21:00", "22:30", "15:00"], "../img/desayuno2.jpg", []),
];
...
var quitarRepetidos = function(array){
    var horarios = array.filter(function(elem, index, self) {
        return index === self.indexOf(elem);
    });
